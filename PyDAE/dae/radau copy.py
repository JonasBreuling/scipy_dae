...
def solve_collocation_system(fun, t, y, h, Z0, scale, tol,
    ...
    for k in range(NEWTON_MAXITER):
        ...
        f_real = h * F.T.dot(TI_REAL) - M_real * mass_matrix.dot(W[0])
        # TODO: Move on here for all complex eigenvalue paris
        f_complex = h * F.T.dot(TI_COMPLEX) - M_complex * mass_matrix.dot(W[1] + 1j * W[2])
        ...
        while not step_accepted:
            ...
            while not converged:
                if LU_real is None or LU_complex is None:
                    ...
                    LU_real = self.lu(MU_REAL * self.mass_matrix - h * J)
                    LU_complex = self.lu(MU_COMPLEX * self.mass_matrix - h * J)
            ...
            error = self.solve_lu(LU_real, f + self.mass_matrix.dot(ZE)) * h
            ...
            if rejected and error_norm > 1: # try with stabilised error estimate
                ...
                error = self.solve_lu(LU_real, self.fun(t, y + error) + self.mass_matrix.dot(ZE)) * h
