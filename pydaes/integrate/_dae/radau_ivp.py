...
from scipy.sparse import csc_matrix, csr_matrix, coo_matrix, issparse, eye, diags, block_diag
...
# TODO: 
# - Incorporate improved error estimate proposed by de Swart and Söderlind, 
...
class RadauDAE(DaeSolver):
    ...
    def __init__(self, fun, t0, y0, yp0, t_bound, max_step=np.inf,
        ...
        # create mass matrix for DAE formulation
        n = self.n
        N = 2 * n
        data = np.ones(n)
        row_ind = col_ind = np.arange(n)
        self.mass_matrix = csr_matrix((data, (row_ind, col_ind)), shape=(N, N))
